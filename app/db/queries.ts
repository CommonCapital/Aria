...
/**
 * Fetches recent agent run summaries to provide context for Aria's memory.
 */
export async function getAriaContext(userId: string, limit: number = 5) {
    const runs = await db
        .select({
    ...
    return runs.map(r => `[${r.completedAt?.toLocaleDateString()}]: ${r.summary}`).join("\n");
}