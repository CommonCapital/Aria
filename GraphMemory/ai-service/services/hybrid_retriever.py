...
async def retrieve(query: str, user_id: str, session_id: str = None, top_k: int = 12) -> List[Dict[str, Any]]:
    embedding = embed_text(query)

    vector_results, graph_results = await asyncio.gather(
        vector_search(user_id, embedding, top_k),
        graph_search(user_id, embedding, hops=2)
    )
